...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate images - chỉ 1 ảnh vé
    if (!images || images.length !== 1) {
      return NextResponse.json(
        { error: "Vui lòng upload đúng 1 ảnh vé" },
        { status: 400 }
      );
    }

    // Parse qrImage - có thể là array hoặc string JSON
    let qrImageArray: string[] = [];
    if (qrImage) {
      if (Array.isArray(qrImage)) {
        qrImageArray = qrImage;
      } else if (typeof qrImage === 'string') {
        try {
          // Thử parse nếu là JSON string
          const parsed = JSON.parse(qrImage);
          qrImageArray = Array.isArray(parsed) ? parsed : [parsed];
        } catch {
          // Nếu không phải JSON, coi như là string đơn
          qrImageArray = [qrImage];
        }
      } else {
        qrImageArray = [qrImage];
    ...
    // Validate qrImage - bắt buộc, tối đa 5 ảnh
    if (!qrImageArray || qrImageArray.length === 0) {
      return NextResponse.json(
        { error: "Vui lòng upload ít nhất 1 ảnh mã QR (bắt buộc)" },
        { status: 400 }
      );
    }

    if (qrImageArray.length > 5) {
      return NextResponse.json(
        { error: "Tối đa 5 ảnh mã QR" },
        { status: 400 }
      );
    }
    ...
    // Create ticket - đảm bảo qrImage là array
    const ticketData: any = {
      ...
      sellingPrice,
      images,
      qrImage: qrImageArray, // Đảm bảo là array
    ...
    // Log để debug
    console.log("Creating ticket with qrImage type:", Array.isArray(ticketData.qrImage) ? "Array" : typeof ticketData.qrImage);
    console.log("qrImage value:", JSON.stringify(ticketData.qrImage));
  ...
  try {
    ...
    const formattedTickets = tickets
      ...
      .map((ticket: any) => ({
        ...
        price: ticket.sellingPrice,
        originalPrice: ticket.originalPrice,
        location: `${ticket.cinema || ""}, ${ticket.city || ""}`.replace(/^,\s*|,\s*$/g, ""),
        image: ticket.images?.[0] || ticket.moviePoster,
        showDate: ticket.showDate,
        showTime: ticket.showTime,
        expireAt: ticket.expireAt,
        ...
        qrImage: (ticket.status === "sold" && ticket.buyer && ticket.qrImage) 
          ? (Array.isArray(ticket.qrImage) 
              ? ticket.qrImage.filter((img: any) => img && typeof img === 'string' && img.trim() !== '')
              : (ticket.qrImage && typeof ticket.qrImage === 'string' && ticket.qrImage.trim() !== '' ? [ticket.qrImage] : [])) 
          : undefined,
        buyer: ticket.buyer?._id?.toString() || ticket.buyer?.toString(),
        buyerEmail: ticket.buyer?.email || (ticket.buyer && typeof ticket.buyer === 'object' ? (ticket.buyer as any).email : undefined),
