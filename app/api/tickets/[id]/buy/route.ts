  ...
  try {
    ...
    try {
      await dbSession.withTransaction(async () => {
        ...
        // Set autoDeleteAt: 10 ngày sau ngày mua
        const autoDeleteDate = new Date();
        autoDeleteDate.setDate(autoDeleteDate.getDate() + 10);
        ticket.autoDeleteAt = autoDeleteDate;
      ...
      const hasTicketCode = ticket.ticketCode && ticket.ticketCode.trim().length > 0;
      const qrImages = Array.isArray(ticket.qrImage) ? ticket.qrImage : (ticket.qrImage ? [ticket.qrImage] : []);
      const hasQrImage = qrImages.length > 0;
      ...
      return NextResponse.json({
        ...
        ticket: {
          ...
          qrImage: hasQrImage ? qrImages : undefined,
        },
      });
    } finally {
